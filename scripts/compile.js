...
const arrayOfThingsToAddFromOutsideSrc = ["package.json", ".env"];
...
const process = async () => {
  try {
    ...
    for (const file of arrayOfThingsToAddFromOutsideSrc) {
      const sourceFilePath = path.join(sourceDir, file);
      const destFilePath = path.join(destDir, file.split("/")[file.split("/").length - 1]);
